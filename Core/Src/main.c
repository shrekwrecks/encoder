...
//	  uint8_t buffer[2];
//	  uint8_t reg = 0x0E; // Starting at ANGLE_H
//	  uint16_t angle = 0;
//
//	  HAL_I2C_Master_Transmit(&hi2c1, 0x36 << 1, &reg, 1, 100);
//	  HAL_I2C_Master_Receive(&hi2c1, 0x36 << 1, buffer, 2, 100);
//
//	  // Combine the two bytes
//	  angle = ((uint16_t)buffer[0] << 8) | buffer[1];
//	  angle = angle & 0x0FFF; // Only 12-bit data is valid
//
//	  printf("Angle (raw 12-bit): %u\r\n", (angle * 360)/4096);
//	  if (result == AS5600_ERROR_SUCCESS) {
//	          printf("AS5600 status: 0x%02X\r\n", p_status);  // Print status as hex
//	      } else {
//	          printf("Failed to read status! Error: %d\r\n", result);
//	      }
    /* USER CODE BEGIN 3 */
  }
  /* USER CODE END 3 */
